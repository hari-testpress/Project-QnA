...
from questions.views import AnswerUpdateView
...
class QuestionUpdateViewTests(TestCase):
    def setUp(self):
        ...
        self.answer.save()
        self.url = reverse(
            "questions:update-answer", args=[self.question.id, self.answer.id]
        )
    ...
    def test_url_resolves_view_class_correctly(self):
        ...
        self.assertEquals(view.func.view_class, AnswerUpdateView)
    ...
    def test_update_the_answer_correctly(self):
        self.client.post(self.url, {"text": "updated answer"})
        updated_answer = Answer.objects.get(id=self.answer.id)
        self.assertEqual(updated_answer.text, "updated answer")
